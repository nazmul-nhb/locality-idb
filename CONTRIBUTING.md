...
- `demo/`: Vite demo app used for manual testing.
...
There are currently no automated tests. Please validate changes by:

- Running TypeScript checks: `pnpm run typecheck`
- Verifying behavior in the demo app: `pnpm run dev` (or `pnpm run build:demo`)

### Optional Developer Tools

> [nhb-toolbox](https://www.npmjs.com/package/nhb-toolbox) is a collection of utility functions and classes for any JavaScript/TypeScript project. `locality-idb` uses a few helpers internally, and the build bundles only the needed pieces (tree-shaken) into `dist/` for published builds.

These scripts use [nhb-scripts](https://www.npmjs.com/package/nhb-scripts), `prettier` and `eslint` for code quality and developer experience:
